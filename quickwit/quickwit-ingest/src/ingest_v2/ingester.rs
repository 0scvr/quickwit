...
pub(super) struct IngesterState {
    ...
    pub replication_streams: HashMap<NodeId, ReplicationStreamTaskHandle>,
...
impl Ingester {
    ...
    async fn init_replication_client(
        ...
        entry.insert(replication_client_task_handle);
        Ok(())
    }
}
    ...
    async fn persist(
        ...
        for (follower_id, subrequests) in replicate_subrequests {
            ...
            let replication_client = state_guard
                .replication_streams
                .get(&follower_id)
                .expect("replication client should be initialized")
                .replication_client();
            replicate_futures
