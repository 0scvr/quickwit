...
impl SoloShard {
    pub fn new(shard_state: ShardState, replication_position_inclusive: Position) -> Self {
        let mut solo_shard = Self::default();
        solo_shard.shard_state = shard_state;
...
impl IngesterShard {
    pub fn close(&mut self) {
        match self {
            IngesterShard::Primary(primary_shard) => primary_shard.shard_state = ShardState::Closed,
            IngesterShard::Replica(replica_shard) => replica_shard.shard_state = ShardState::Closed,
            IngesterShard::Solo(solo_shard) => solo_shard.shard_state = ShardState::Closed,
        }
    }
