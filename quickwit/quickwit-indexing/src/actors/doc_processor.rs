...
impl From<OtlpTraceError> for DocProcessorError {
    fn from(_error: OtlpTraceError) -> Self {
        DocProcessorError::ParsingError
    }
}
...
fn try_into_vrl_doc(
    ...
    raw_doc: Bytes,
) -> Result<VrlValue, DocProcessorError> {
    let vrl_doc = match input_format {
...
fn try_into_json_docs(input_format: SourceInputFormat, raw_doc: Bytes) -> JsonDocIterator {
    match input_format {
        SourceInputFormat::Json => match serde_json::from_slice::<JsonObject>(&raw_doc) {
            Ok(json_obj) => JsonDocIterator::from(json_obj),
            Err(error) => JsonDocIterator::from(error),
        },
        SourceInputFormat::OtlpTraceJson => match parse_otlp_spans_json(&raw_doc) {
            Ok(spans) => JsonDocIterator::Spans(spans),
            Err(error) => JsonDocIterator::from(error),
        },
        SourceInputFormat::OtlpTraceProtobuf => match parse_otlp_spans_protobuf(&raw_doc) {
            Ok(spans) => JsonDocIterator::Spans(spans),
            Err(error) => JsonDocIterator::from(error),
        },
        SourceInputFormat::PlainText => {
            ...
            let key = PLAIN_TEXT.to_string();
            let value = match String::from_utf8(raw_doc.to_vec()) {
                Ok(value) => value,
                Err(error) => return JsonDocIterator::from(error),
            };
            map.insert(key, JsonValue::String(value));
            JsonDocIterator::from(map)
        }
    }
}
...
fn parse_raw_doc(
    ...
    let Some(vrl_program) = vrl_program_opt else {
        return try_into_json_docs(input_format, raw_doc);
    };
    match try_into_vrl_doc(input_format, raw_doc)
        .and_then(|vrl_doc| vrl_program.transform_doc(vrl_doc))
        .and_then(|transformed_vrl_doc| {
            serde_json::to_value(transformed_vrl_doc).map_err(DocProcessorError::from)
        }) {
        Ok(json_value) => JsonDocIterator::from(json_value),
        Err(error) => JsonDocIterator::from(error),
    }
}
...
enum JsonDocIterator {
    One(Option<Result<JsonObject, DocProcessorError>>),
    Spans(JsonSpanIterator),
}

impl From<JsonValue> for JsonDocIterator {
    fn from(json_value: JsonValue) -> Self {
        match json_value {
            JsonValue::Object(json_obj) => Self::from(json_obj),
            _ => Self::from(DocProcessorError::ParsingError),
        }
    }
}

impl From<JsonObject> for JsonDocIterator {
    fn from(json_obj: JsonObject) -> Self {
        Self::One(Some(Ok(json_obj)))
    }
}

impl<E> From<E> for JsonDocIterator
where E: Into<DocProcessorError>
{
    fn from(error: E) -> Self {
        Self::One(Some(Err(error.into())))
    }
}

impl Iterator for JsonDocIterator {
    type Item = Result<JsonObject, DocProcessorError>;

    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::One(opt) => opt.take(),
            Self::Spans(spans) => match spans.next()? {
                JsonValue::Object(span) => Some(Ok(span)),
                _ => Some(Err(DocProcessorError::ParsingError)),
            },
        }
    }
}
...
impl DocProcessorCounters {
    ...
    pub fn record_error(&self, error: DocProcessorError, num_bytes: u64) {
        let label = match error {
            ...
            DocProcessorError::SchemaError => {
                self.num_schema_errors.fetch_add(1, Ordering::Relaxed);
                "schema_error"
            }
            #[cfg(feature = "vrl")]
            DocProcessorError::TransformError(_) => {
...
impl DocProcessor {
    ...
    fn extract_timestamp(
        ...
        let timestamp = doc
            ...
            .ok_or(DocProcessorError::SchemaError)?;
        Ok(Some(timestamp))
    }
        ...
        for json_doc_result in parse_raw_doc(self.input_format, raw_doc, transform_opt) {
            ...
            match processed_doc_result {
                Ok(processed_doc) => {
                    ...
                    self.counters.record_valid(num_bytes as u64);
                }
                Err(error) => {
    ...
    fn process_json_doc(
        ...
        let (partition, doc) = self
            ...
            .map_err(|error| {
                warn!(error=?error);
                match error {
                    DocParsingError::RequiredField(_) => DocProcessorError::SchemaError,
